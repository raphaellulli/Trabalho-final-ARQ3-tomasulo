...
const AvancarInstrucoes: React.FC = () => {
  ...
  const avancarInstrucoes = () => {
    ...
    if (
      ...
      arrEstacaoReserva.value.every((e) => e.ocupada === false)
    ) {
    ...
    const aux = arrInstrucoes.value.some(
      (i) => i.id === instrucaoAtual?.id && i.descartada
    );
    if (
      ...
      if (instrucaoAtual.nome === "Add" || instrucaoAtual.nome === "Sub") {
        estacaoReservaVazia = arrEstacaoReserva.value.find(
          (er) => er.TipoRegistrador === TipoRegistrador.Inteiro && !er.ocupada
        );
      } else if (
        ...
        estacaoReservaVazia = arrEstacaoReserva.value.find(
          (er) =>
            er.TipoRegistrador === TipoRegistrador.Flutuante && !er.ocupada
        );
      } else if (instrucaoAtual.nome === "Ld") {
        estacaoReservaVazia = arrEstacaoReserva.value.find(
          (er) => er.TipoRegistrador === TipoRegistrador.Load && !er.ocupada
        );
      } else if (instrucaoAtual.nome === "St") {
        estacaoReservaVazia = arrEstacaoReserva.value.find(
          (er) => er.TipoRegistrador === TipoRegistrador.Store && !er.ocupada
        );
      } else if (instrucaoAtual.nome === "Jump") {
        estacaoReservaVazia = arrEstacaoReserva.value.find(
          (er) => er.TipoRegistrador === TipoRegistrador.Jump && !er.ocupada
        );
      }
      ...
      if (estacaoReservaVazia !== undefined) {
        ...
        const destinoIndex = arrBufferAux.findIndex(
          (b) => b.idInstrucao === instrucaoAtual.entrada1
        );
        estacaoReservaVazia.destino =
          destinoIndex !== -1
            ? (destinoIndex + 1).toString()
            : instrucaoAtual.entrada1;
        if (
          ...
          estacaoReservaVazia.A = `${instrucaoAtual.entrada2}${
            instrucaoAtual.entrada3
              ? " + (" + instrucaoAtual.entrada3 + ")"
              : ""
          }`;
          estacaoReservaVazia.registradorSendoUtilizado =
            instrucaoAtual.entrada3;
        } else if (
              ...
              (er.destino ===
                (
                  arrBufferAux.findIndex(
                    (b) => b.idInstrucao === instrucaoAtual.entrada2
                  ) + 1
                ).toString() ||
                er.destino === instrucaoAtual.entrada2)
          );
          const estacaoPendenteEnt3 = arrEstacaoReserva.value.find(
            (er) =>
              ...
              (er.destino ===
                (
                  arrBufferAux.findIndex(
                    (b) => b.idInstrucao === instrucaoAtual.entrada3
                  ) + 1
                ).toString() ||
                er.destino === instrucaoAtual.entrada3)
          );
          if (
            estacaoPendenteEnt2 &&
            !arrInstrucoes.value.find(
            ...
            estacaoReservaVazia.Vj = arrBufferAux
              .findIndex((b) => b.idInstrucao === instrucaoAtual.entrada2)
              .toString();
            estacaoReservaVazia.Vj =
              estacaoReservaVazia.Vj === "-1"
                ? instrucaoAtual.entrada2
                : estacaoReservaVazia.Vj + 1;
          }
          if (
            estacaoPendenteEnt3 &&
            ...
            estacaoReservaVazia.Vk = arrBufferAux
              .findIndex((b) => b.idInstrucao === instrucaoAtual.entrada3)
              .toString();
